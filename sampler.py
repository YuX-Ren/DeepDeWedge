...
if __name__ == "__main__":
    ...
    model_checkpoint_file = "./tutorial_project/logs/version_146/checkpoints/epoch/epoch=4999.ckpt"
    gpu = 0
    device = "cpu" if gpu is None else f"cuda:{gpu}"
    latents = torch.randn(1, 96, 96, 96).to(device)
    print(f"Loading model from {model_checkpoint_file}")
    net = (
        LitUnet3D.load_from_checkpoint(model_checkpoint_file).to(device).eval()
    )
    output = edm_sampler(net, latents).detach().cpu()
    save_mrc_data(output, "output.mrc")